...
- Alice initie cet échange atomique. Elle crée une transaction $mA$ qui envoie 1 BTC vers Bob. Elle crée une signature $sA$ qui permet de valider cette transaction grâce à sa clé privée $pA$ ($PA = pA \cdot G$), et en utilisant un nonce $nA$ et un secret $t$ ($NA = nA \cdot G$ et $T = tG$) : $$sA = nA + t + H(NA + T \parallel PA \parallel mA) \cdot pA$$
